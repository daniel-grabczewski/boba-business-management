        ...
        <div className="flex flex-col items-center w-full">
          <div className="w-full max-w-[750px] mx-auto flex flex-col gap-4">
            {getPaginatedProducts().map((product) => (
              <div
                key={product.id}
                className={`box-border rounded-md flex items-center justify-between p-4 sm:p-6 ${
                ...
                style={{ height: '200px' }}
              >
                <div className="w-[150px] h-[150px] flex justify-center items-center px-2 mr-[5px]">
                  <Link
                ...
                <div className="flex-col w-1/4">
                  {product.stock < lowStockThreshold && (
                    <Link
                      ...
                      onMouseEnter={() => setHoveredProductId(product.id)}
                      onMouseLeave={() => setHoveredProductId(null)}
                      className="text-xl font-bold block cursor-pointer mt-2 pb-24"
                      style={{
                        color:
